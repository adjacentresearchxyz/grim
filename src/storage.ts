...
export class CloudflareKVStorage implements SessionStorage {
  ...
  async get(key: string): Promise<SessionData | undefined> {
    try {
      ...
      logger.error(`Error retrieving session for key: ${key}`, { error });
      return undefined;
    }
  }
  ...
  async set(key: string, data: SessionData): Promise<void> {
    try {
      ...
      logger.error(`Error storing session for key: ${key}`, { error });
      throw error;
    }
  }
  ...
  async delete(key: string): Promise<void> {
    try {
      ...
      logger.error(`Error deleting session for key: ${key}`, { error });
      throw error;
    }
  }
}